...
public partial class Pages_FeePaymentModule_ManageStudentFeeMapping : BasePage
{
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(ddlClass.SelectedValue))
            {
                ddlFeeHead.DataSource = new dalCommon().GetByQuery("select * from acc_FeeHead where IsActive = 'Active' AND ChargeBy = 'System' and (ClassId = " + ddlClass.SelectedValue + " OR ClassId is null) " +
                    "and (IsActive_ForWaiver != 'Active' and FcCode not like '%Course Fee%' and FcCode not like '%Transport Fee%') " +
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        foreach (GridViewRow row in gvList.Rows)
        {
            ...
            var lbl_Mapping_FullAmount_New = (Label)row.FindControl("lbl_Mapping_FullAmount_New");
            var lbl_Mapping_AppliedAmount_New = (Label)row.FindControl("lbl_Mapping_AppliedAmount_New");
            ...
            if (cbx_WillSave.Checked == true && !string.IsNullOrEmpty(lbl_Fee_Id.Text))
            {
                var resRow = dal.StudentFeeMapping_InsertOrUpdate(
                    ...
                    WaiverPercentage: -1,
                    ConsumptionUnit: -1,
                    FullAmount: Convert.ToDecimal(lbl_Mapping_FullAmount_New.Text),
                    AppliedAmount: Convert.ToDecimal(lbl_Mapping_AppliedAmount_New.Text),
