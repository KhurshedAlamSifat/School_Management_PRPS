...
public partial class Pages_FeePaymentModule_ManageStudentFeeMappingWithWaiver : BasePage
{
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        foreach (GridViewRow row in gvList.Rows)
        {
            ...
            var tbx_NewPercentage = (TextBox)row.FindControl("tbx_NewPercentage");
            ...
            if (cbx_WillSave.Checked == true && !string.IsNullOrEmpty(lbl_Fee_Id.Text))
            {
                var resRow = dal.StudentFeeMapping_InsertOrUpdate(
                    ...
                    WaiverPercentage: !string.IsNullOrEmpty(tbx_NewPercentage.Text) ? Convert.ToDecimal(tbx_NewPercentage.Text) : -1,
                    ConsumptionUnit: -1,
                    FullAmount: Convert.ToDecimal(lbl_Mapping_FullAmount_New.Text), 
                    AppliedAmount: Convert.ToDecimal(tbx_Mapping_AppliedAmount_New.Text),
