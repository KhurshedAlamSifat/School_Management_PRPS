    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        foreach (RepeaterItem item in rptStudent.Items)
        {
            ...
            if (chkrow.Checked)
            {
                ...
                dt.Rows.Add(Convert.ToInt32(hdnObtainMarksEntry.Value), Convert.ToInt32(hdnStudentId.Value), Convert.ToInt32(ddlSubject.SelectedValue), theory, objective, practical, sbs, ct1, ct2, ct3);
            }

        }
        if (count > 0)
        {
            objMarks.UpdateMarks(dt, Convert.ToInt32(ddlYear.SelectedItem.Text), Convert.ToDouble(tbxMarksOutOf.Text), Convert.ToInt32(ddlExamType.SelectedValue), ctOUtOf, SessionManager.SessionName.UserName);
            BindData();
