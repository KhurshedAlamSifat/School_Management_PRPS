...
public partial class Report_Viewer_Dues : System.Web.UI.Page
{
    ...
    protected string GetCriteria()
    {
        ...
        {
            criteria += " and p.Year=" + ddlYear.SelectedValue;
        }
        if (ddlMonth.SelectedValue != "0")
        {
            criteria += " and p.Month=" + ddlMonth.SelectedValue;
        }
        if (ddlClass.SelectedValue != "")
        {
            criteria += " and c.ClassId=" + ddlClass.SelectedValue;
        }
        if (ddlGroup.SelectedValue != "")
        {
            criteria += " and c.GroupId=" + ddlGroup.SelectedValue;
        }
        if (ddlShift.SelectedValue != "")
        {
            criteria += " and c.ShiftId=" + ddlShift.SelectedValue;
        }
        if (ddlSection.SelectedValue != "")
        {
            criteria += " and c.SectionId=" + ddlSection.SelectedValue;
        }
        if (ddlReligion.SelectedValue != "")
        {
            criteria += " and sps.ReligionId=" + ddlReligion.SelectedValue;
        }
        if (ddlGender.SelectedValue != "")
        {
            criteria += " and sps.GenderId=" + ddlGender.SelectedValue;
        }
        if (tbxRegNo.Text != "")
        {
            criteria += " and s.RegNo='" + tbxRegNo.Text + "'";
        }
    ...
    {
        DataTable dt = objPayment.GetDues(GetCriteria());
        rpt.DataSource = dt;
        rpt.DataBind();
    }
